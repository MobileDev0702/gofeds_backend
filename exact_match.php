...
if (isset($_POST['user_id'])){
    ...
    $gf_desire_port     =   explode(" ",$usdat['gf_desire_port']);
    
    $query1="SELECT * FROM gf_users where gf_agency='".$usdat['gf_agency']."' and gf_office='".$usdat['gf_office']."' and  gf_id!='".$_POST['user_id']."' and
    	gf_desire_port LIKE '%".$usdat['gf_current_port']."%' and ";
    	$query1 .= "(";
    	$count = count($gf_desire_port)-1;
        foreach($gf_desire_port as $key => $port){
        $query1 .= "gf_current_port	 LIKE '%".$port."%'
    	  ";
    	  if($key < $count){$query1.= ' OR ';}
    	}
		$query1 .= ")"; 
